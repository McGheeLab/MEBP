...
from PySide6.QtGui import QRegularExpressionValidator, QValidator, QColor
...
class PrintSettingsWidget(QWidget):
    def __init__(self, print_manager, parent=None):
        ...
        self.ink_well_widgets = []  # dynamically created ink well widgets
        self.standard_syringe_types = {}  # loaded automatically from "Syringes.json"
        self.initUI()
    ...
    def initUI(self):
        ...
        for pump in ["p1", "p2", "p3"]:
            ...
            # Standard Type dropdown – automatically populated from Syringes.json.
            ...
            # Cell Type dropdown (populated from ink wells).
        ...
        # ----- Bottom: Load All / Save All Settings -----
        ...
        # Overall Apply Settings button.
        apply_button = QPushButton("Apply Settings")
        apply_button.clicked.connect(self.apply_settings)
        main_layout.addWidget(apply_button)

    # ----- Standard Syringe Types Loader (from file "Syringes.json") -----
    ...
    # ----- Update Syringe Cell Type Options -----
    ...
    def validate_syringe_tab(self):
        # If no ink wells, then valid types is {"None"}.
        if len(self.ink_well_widgets) == 0:
        ...
        for pump, inputs in self.syringe_inputs.items():
            ...
            # Allow "None" type without positive diameter/length.
            if cell_type != "None":
    ...
    # ----- Ink Well Properties: Add / Remove -----
    ...
    def apply_settings(self):
        ...
        if errors:
            error_text = "\n".join(errors)
            QMessageBox.warning(self, "Apply Error", f"Cannot apply settings due to the following errors:\n{error_text}")
            return
        # If all validations pass, update the PrintManager.
        ...
        new_ink_wells = {}
        for i, ink_widget in enumerate(self.ink_well_widgets, start=1):
            loc_text = ink_widget["location"].text().strip()
            if not loc_text:
                QMessageBox.warning(self, "Validation Error", f"Ink well {i}: Location cannot be empty.")
                return
            ink_well = InkWell()
            ink_well.set_location(loc_text)
            ink_well.set_cell_type(ink_widget["cell_type"].text().strip())
            ink_well.set_chilled(ink_widget["chilled"].isChecked())
            ink_well.set_volume(ink_widget["volume"].value())
