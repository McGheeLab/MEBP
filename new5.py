...
class Stages:
    ...
    UPDATE_INTERVAL_ZP = 0.1  # Time between updates (100 ms)
    ...
    def __init__(self, waypoints, simulate=False, Kp=1.0, Ki=0.0, Kd=0.0):
        ...
        self.Ki1 = 0.0
        self.Kd1 = 0.0
        # Initialize PID controller variables for XY stage
        self.error_sum_x = 0.0
        self.error_sum_y = 0.0
        self.last_error_x = 0.0
        self.last_error_y = 0.0

        # Initialize PID controller variables for ZP stage
        self.error_sum_z = 0.0
        self.error_sum_p1 = 0.0
        self.error_sum_p2 = 0.0
        self.error_sum_p3 = 0.0

        self.last_error_z = 0.0
        self.last_error_p1 = 0.0
        self.last_error_p2 = 0.0
        self.last_error_p3 = 0.0

    def calculate_pid(self, error, delta_t, axis):
        """Calculate PID control output for a given axis."""
        # Get the integral and derivative terms for the axis
        if axis == 'z':
            error_sum = self.error_sum_z
            last_error = self.last_error_z
        elif axis == 'p1':
            error_sum = self.error_sum_p1
            last_error = self.last_error_p1
        elif axis == 'p2':
            error_sum = self.error_sum_p2
            last_error = self.last_error_p2
        elif axis == 'p3':
            error_sum = self.error_sum_p3
            last_error = self.last_error_p3
        else:
        ...
        I = self.Ki1 * error_sum

        # Derivative term
        D = self.Kd1 * (error - last_error) / delta_t
        ...
        if axis == 'z':
            self.error_sum_z = error_sum
            self.last_error_z = error
        elif axis == 'p1':
            self.error_sum_p1 = error_sum
            self.last_error_p1 = error
        elif axis == 'p2':
            self.error_sum_p2 = error_sum
            self.last_error_p2 = error
        elif axis == 'p3':
            self.error_sum_p3 = error_sum
            self.last_error_p3 = error
    ...
    def move(self, plot_title, interpolation_type="linear", plot=False):
        ...
        while True:
            ...
            if current_time >= next_update_time_zp:
                ...
                # Desired positions
                desired_z = interpolated_values_zp['z']
                desired_p1 = interpolated_values_zp['p1']
                desired_p2 = interpolated_values_zp['p2']
                desired_p3 = interpolated_values_zp['p3']

                # Get current positions from the ZP stage
                zp_current_pos = self.zp_manager.get_position()
                current_z = zp_current_pos[0]  # x_pos corresponds to z
                current_p1 = zp_current_pos[1]  # y_pos corresponds to p1
                current_p2 = zp_current_pos[2]  # z_pos corresponds to p2
                current_p3 = zp_current_pos[3]  # e_pos corresponds to p3

                # Compute errors
                error_z = desired_z - current_z
                error_p1 = desired_p1 - current_p1
                error_p2 = desired_p2 - current_p2
                error_p3 = desired_p3 - current_p3

                # Append errors to lists
                error_list_z.append(error_z)
                error_list_p1.append(error_p1)
                error_list_p2.append(error_p2)
                error_list_p3.append(error_p3)

                # Compute PID control outputs
                control_output_z = self.calculate_pid(error_z, delta_t_zp, 'z')
                control_output_p1 = self.calculate_pid(error_p1, delta_t_zp, 'p1')
                control_output_p2 = self.calculate_pid(error_p2, delta_t_zp, 'p2')
                control_output_p3 = self.calculate_pid(error_p3, delta_t_zp, 'p3')

                # Optionally limit the control outputs
                max_correction = 1.0  # Set maximum correction per update
                control_output_z = max(min(control_output_z, max_correction), -max_correction)
                control_output_p1 = max(min(control_output_p1, max_correction), -max_correction)
                control_output_p2 = max(min(control_output_p2, max_correction), -max_correction)
                control_output_p3 = max(min(control_output_p3, max_correction), -max_correction)

                # Compute target positions
                target_z = current_z + control_output_z
                target_p1 = current_p1 + control_output_p1
                target_p2 = current_p2 + control_output_p2
                target_p3 = current_p3 + control_output_p3

                # Create axes dictionary for movecommand
                zp_axes = {
                    'X': target_z,
                    'Y': target_p1,
                    'Z': target_p2,
                ...
                # Send move command to ZP stage
                self.zp_manager.movecommand(zp_axes)

                # Collect data for plotting
                ideal_path_z.append(desired_z)
                ideal_p1.append(desired_p1)
                ideal_p2.append(desired_p2)
                ideal_p3.append(desired_p3)
    ...
    def calculate_velocity_to_follow_path(current_x, current_y, future_target_x, future_target_y, delta_t, max_velocity):
        ...
        # Limit velocities to maximum allowed values
        velocity_magnitude = np.hypot(required_vx, required_vy)  # Calculate magnitude of velocity vector
        if velocity_magnitude > max_velocity:
            scaling_factor = max_velocity / velocity_magnitude
...
class ZPStageSimulator:
    def __init__(self):
        ...
        self.processing_time_per_command = 0.1  # Simulate time to process each command
