...
class ZPStageManager:
    ...
    def send_data(self, data: str) -> None:
        ...
        Handles both real serial and simulator transparently.
        """
        if self.serial is None:
        ...
        # Retry logic for serial port not ready
        for attempt in range(5):
            if hasattr(self.serial, "is_open") and not self.serial.is_open:
                if attempt < 4:
                    time.sleep(0.01)
                    continue
                logger.error("Serial port still not open after retries")
                return
            break
        ...
        try:
            ...
            self.serial.flush()
        except (serial.SerialException if serial else OSError) as e:
            logger.error(f"ZP send_data error: {e}")

    def receive_data(self) -> str:
        """Read all available response data from the printer."""
        time.sleep(0.01)
        try:
