    ...
    def run(self):
        ...
        def xy_loop():
            for i in range(len(self.xy_times) - 1):
                ...
                # run until it's time for next outer setpoint
                while True:
                    now = time.time()
                    if now >= target_wall:
                        break
                    ...
                    time_remain = target_wall - now
                    # avoid division by zero
                    if time_remain < 1e-3:
                        vx, vy = 0.0, 0.0
                    else:
                        # predictive velocity to hit (x_target,y_target) exactly at target_wall
                        vx = (x_target - x_act) / self.dt_xy
                        vy = (y_target - y_act) / self.dt_xy

                    # send velocity to stage
                    self.xy_mgr.move_stage_at_velocity(vx, vy)

                    # optional logging
                    print(f"t={now-start_wall:.2f}s | "
                        f"Tgt=({x_target:.1f},{y_target:.1f}) | "
                        f"Act=({x_act:.1f},{y_act:.1f}) | "
                        f"Vel=({vx:.1f},{vy:.1f})")
