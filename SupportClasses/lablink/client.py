...
from .protocol import CHUNK, DEFAULT_TIMEOUT_S, H_META, H_NODE, H_SHA, H_TOKEN
...
class LabLinkClient:
    def __init__(self, base_url: str, token: str, timeout: float = DEFAULT_TIMEOUT_S,
        ...
        """node_id: send this machine's own identity alongside its token.

        Optional, and absent by default, so every existing caller keeps sending
        exactly the same request it always did. When set, `token` is that node's
        token rather than the shared site token, and the hub can attribute the
        request and revoke this one machine.
        """
    ...
    def _open(self, method: str, url: str, data=None, headers: dict | None = None):
        ...
        if self.node_id:
            req.add_header(H_NODE, self.node_id)
