...
def _default_path() -> Path:
    """Per-machine calibration file.

    Lives in ``config/hardware/`` beside the camera calibrations and the
    microscope config — a sensor offset is a property of THIS rig's
    thermistors, not of a swappable print setup. ``MEBP_INCUBATOR_CAL_DIR``
    redirects it for test isolation (the house env-override pattern).
    """
    d = os.environ.get("MEBP_INCUBATOR_CAL_DIR")
    if d:
        return Path(d) / "incubator_calibration.json"
    here = Path(__file__).resolve()
    for cand in here.parents:
        if (cand / "SupportClasses").is_dir():
            return cand / "config" / "hardware" / "incubator_calibration.json"
    return Path("config/hardware") / "incubator_calibration.json"
...
class CalibrationStore:
    ...
    def __init__(self, path: Path | None = None):
        # Resolved at CONSTRUCTION, not import, so a test can set the env
        # override in setUp() and still get an isolated store.
