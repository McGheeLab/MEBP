...
def available_ports() -> list[dict]:
    ...
    Goes through :mod:`._serial_helpers`, which reuses the app's ``SerialUtils``
    ...
    """
    from ._serial_helpers import list_serial_ports
    try:
...
def ranked_ports() -> list[tuple[str, int, str]]:
    """
...
def suggest_ports() -> list[str]:
    """
    Ordered port suggestions for the UI combo, best guess first.

    Deliberately excludes cached ports that are not currently present.
    """
    return [dev for dev, _score, _desc in ranked_ports()]
...
def detect_marlin(
    *,
    ...
    candidates = ports if ports is not None else suggest_ports()
