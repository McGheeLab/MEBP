# MEBP v7.3.7 — Architecture Reference

**Multi-Extrusion Bioprinting Platform**
**Version 7.3.7 | March 2026**
...
3. **Hardware-first configuration** — Page 0 (Hardware Setup) must be completed before any other page is accessible. `HardwareConfig` is the single source of truth for all uL <-> mm conversions.
4. **uL-native pump control** — All user-facing pump values are in microliters. Conversion to mm (for Marlin firmware) happens exclusively inside `StageController.move_pump_uL()` via `SyringeSpec`.
...
8. **Correct unit terminology** — XY stage position readout is in um (not microsteps). The `xy_position_scale` factor converts between stage readout units and um (identity 1.0 for ProScan). The term "microsteps" is reserved for actual physical motor microsteps.
9. **DPI-aware GUI** — All pixel and font-size values in the GUI are wrapped with scaling functions (`s()` for pixels, `sf()` for points) from `gui/scaling.py`. The theme QSS is generated dynamically via `build_theme(scale_factor)`. The UI never shrinks below its 96-DPI design size, only grows on high-DPI screens. Mode page sidebar buttons follow the same DPI-scaling pattern as the main sidebar buttons in `styles.py`, using `sf()` for font sizing with cross-platform emoji font-family fallbacks.
...
│   │   ├── syringes.json                # Hamilton syringe catalog (25–1000 uL)
│   │   ├── objectives.json              # Per-camera objective um/px calibrations  <- v7.3.4
...
│   ├── SafetyLimits.py                  # Software endstops + flow-rate clamping (um labels)  <- v7.3.5
│   ├── Settings.py                      # JSON-backed persistent app config
│   ├── PositionLogger.py                # Timestamped position recording + CSV export
│   ├── HardwareConfig.py                # Central config: pumps, inks, needles, uL<->mm
...
│   ├── ObjectiveCalibration.py          # Per-camera objective um/px store      <- v7.3.4
...
│   │   ├── print_setup.py              # Sub-page: Print setup (4 tabs) — DPI-scaled tab bar  <- v7.3.7
│   │   ├── print_workspace.py          # Tab 1: Hardware summary
│   │   ├── print_objects.py            # Tab 2: Object designer
│   │   ├── print_well_setup.py         # Tab 3: Well assignments — SECTION_TITLE_STYLE from gui.styles  <- v7.3.7
...
│   │   └── mode_page.py               # ModePage base class — DPI-scaled sidebar buttons  <- v7.3.7
│   └── widgets/
...
└── coding plans/                        # Development documentation
    ...
    │   ├── ARCHITECTURE_V736.md
    │   └── ARCHITECTURE_V737.md         # <- This document
    ├── Readmes/
    └── Update plans/
        ├── MEBP_v734_to_v735_UPDATE.md
        ├── MEBP_v735_to_v736_UPDATE.md
        └── MEBP_v736_to_v737_UPDATE.md
```
...
├── SafetyLimits (software endstops for all axes, um units)
...
```
HardwareConfig (central authority for uL<->mm conversions)
├── NeedleSpec (gauge, OD/ID, channel count)
├── PumpChannelConfig x 3 (P1, P2, P3)
...
│   │   ├── PrintSetupPage              — 4-tab print designer (DPI-scaled tab bar)  <- v7.3.7
...
├── Top Bar (status dots: XY, ZP, Xbox + global position in um)
...
Single source of truth for hardware state. All uL<->mm conversions route through here.
...
**Coordinate systems:**
- All `_predicted_positions` and `_calibrated_positions` are in **absolute stage um** (origin at stage mechanical limit). Zero-reference (`controller.zero_position`) is applied only when displaying values to the user. Calibrated positions remain valid even if the zero reference changes.
...
- Three loading modes: TIFF stack, numbered image sequence, single image x N layers
...
├── DOF model: opacity = exp(-0.5*(defocus/dof_hw)^2)
...
```
ZPStageSimulator
├── Physics loop (100 Hz): motor counts tracked (delta_mm x steps_per_mm)
├── G-code parsing:
...
├── stage_to_um(readout, xy_position_scale) → um
...
**Terminology:** The ProScan reports positions in um natively. The `xy_position_scale` factor is 1.0 (identity) for ProScan stages. The term "microsteps" is NOT used for position readout — only for actual physical motor microsteps.
...
Scaling applied across 20+ files:
  ...
  - gui/pages/mode_page.py: sidebar button font sizing via sf() with cross-platform
    emoji font-family fallbacks (Segoe UI Emoji, Apple Color Emoji, Noto Color Emoji)  <- v7.3.7
  - gui/pages/print_setup.py: tab bar padding/margins DPI-scaled via _sc()  <- v7.3.7
...
| Terminators | `\r\n` |
| Encoding | ASCII |
| Position unit | um (stage reports in whole microns; `xy_position_scale` = 1.0) |

**Key commands** (from `config/controllers/proscan_iii.json`):
- `P` → query position → `x,y\r`
- `G x,y` → move absolute (in um)
- `GR dx,dy` → move relative (in um)
...
#### PrintSetup (`print_setup.py`)

**v7.3.7:** Tab bar padding and margins are now DPI-scaled via `_sc()`, ensuring consistent tab spacing across different screen densities.

#### PrintWellSetup (`print_well_setup.py`)

**v7.3.7:** Fixed import — `SECTION_TITLE_STYLE` is now correctly imported from `gui.styles` (was previously referencing `gui.scaling` where it does not exist).
...
**3 operation modes:**
1. Spheroid pickup: V=(4/3)pi(d/2)^3 x safety_factor → aspirate/dispense
...
### ModePage Base Class (`mode_page.py`) <- v7.3.3

**Purpose:** Base class for mode containers (PrintingModePage, PickPlaceModePage) providing right-side icon column for sub-page switching.

**v7.3.7:** Sidebar button styling now uses DPI-scaled font sizing via `sf()` with cross-platform emoji font-family fallbacks (`Segoe UI Emoji`, `Apple Color Emoji`, `Noto Color Emoji`). Dead code (`_make_sub_icon()`) was removed.
...
Usage in GUI:
    stage_to_um(readout, self._xy_position_scale) → display in um
...
### 9.1 Stage Position <-> um Conversion (v7.3.5)

```
ProScan XY: xy_position_scale = 1.0 (stage speaks um natively)
...
Syringe: barrel cross-section determines uL<->mm
  area_mm2 = pi x (barrel_id_mm / 2)^2
  mm = uL / area_mm2 / 1000
  uL = mm x area_mm2 x 1000
```
...
1. **Coarse sweep** (0.15mm steps): Lower needle, capture focus score. Detect needle entry when score > 2x baseline.
...
```json
{
  ...
  "pumps": {
    "P1": {
      "pump_id": "P1",
      "enabled": true,
      "syringe": { "name": "Hamilton 250uL", "volume_uL": 250 },
      "inks": [
...
- `parameters.xy_position_scale` — scale factor between readout units and um (1.0 for ProScan)
...
| v7.3.3 | Mode-based navigation restructure (ModePage base class, right-side icon sub-nav), Printing mode container, Pick & Place mode (PickAndPlaceManager, ImageStitcher, 3 operation modes), CameraManager shared feeds, camera um/px empirical calibration, relaxed needle detection with interactive edge refinement |
...
| v7.3.7 | **DPI-scaling fixes:** Mode page sidebar buttons now use `sf()` for DPI-scaled font sizing with cross-platform emoji font-family fallbacks; dead code (`_make_sub_icon()`) removed from `mode_page.py`. Print setup tab bar padding/margins DPI-scaled via `_sc()`. **Import fix:** `print_well_setup.py` corrected to import `SECTION_TITLE_STYLE` from `gui.styles` instead of `gui.scaling`. |
